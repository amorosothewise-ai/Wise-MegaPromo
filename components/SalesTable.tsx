...
const SaleRow = memo(({ 
    ...
    return (
      <tr className="bg-amber-50/50 transition-colors">
        ...
        <td className="px-10 py-4 text-right flex justify-end gap-3">
          <button onClick={onEditSave} className="bg-blue-600 text-white p-3 rounded-xl shadow-lg active:scale-95"><Check size={20} strokeWidth={3}/></button>
          <button onClick={onCancelEdit} className="bg-slate-200 text-slate-600 p-3 rounded-xl shadow-lg active:scale-95"><X size={20} strokeWidth={3}/></button>
        </td>
      </tr>
    );
  }
  ...
  return (
    <tr className={`${isSelected ? 'bg-blue-50/40' : 'hover:bg-slate-50/30'} group transition-all duration-200`}>
      <td className="px-10 py-5">
        <button onClick={() => onSelect(sale.id)} className="min-h-[44px] min-w-[44px] flex items-center justify-center text-slate-200 group-hover:text-slate-400 transition-all">
      ...
      <td className="px-8 py-5 font-black text-red-600 text-sm tracking-tight">{formatMZN(calculateReinvestment(sale.quantity))}</td>
      <td className="px-10 py-5 text-right flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
        <button onClick={() => onInfo(sale)} className="bg-blue-600 text-white p-3 rounded-xl active:scale-95 transition-all"><Info size={20} strokeWidth={2.5}/></button>
        <button onClick={() => onEdit(sale)} className="bg-white text-blue-600 border-2 border-blue-600 p-3 rounded-xl active:scale-95 transition-all"><Pencil size={20} strokeWidth={2.5}/></button>
        <button onClick={() => onDelete(sale.id)} className="bg-red-600 text-white p-3 rounded-xl active:scale-95 transition-all"><Trash2 size={20} strokeWidth={2.5}/></button>
      </td>
    </tr>
  );
});

// Cartão Móvel Memoizado
const MobileSaleCard = memo(({ 
  sale, isSelected, onSelect, onInfo, onEdit, onDelete, idx 
}: { 
  sale: DiamondSale, isSelected: boolean, onSelect: any, onInfo: any, onEdit: any, onDelete: any, idx: number 
}) => {
  const debtValue = calculateReinvestment(sale.quantity);
  return (
    <div className={`bg-white rounded-[2.5rem] p-8 border-2 transition-all shadow-sm active:scale-[0.98] ${isSelected ? 'border-blue-600 ring-[8px] ring-blue-50 shadow-xl' : 'border-slate-200 shadow-sm'}`} 
         style={{ contain: 'layout style', transform: 'translateZ(0)' }}>
      <div className="flex justify-between items-start mb-6">
         <div className="flex items-center gap-4">
            <button onClick={() => onSelect(sale.id)} className="p-2 -ml-2 min-h-[44px] min-w-[44px] flex items-center justify-center">
        ...
        <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center bg-slate-950/70 backdrop-blur-md p-0 sm:p-4">
          <div className="bg-white rounded-t-[2.5rem] sm:rounded-[3rem] shadow-2xl w-full max-w-md overflow-hidden animate-in slide-in-from-bottom duration-300">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-white">
              ...
              <button onClick={() => setSelectedSale(null)} className="p-3 rounded-2xl border-2 border-slate-100"><X size={24} strokeWidth={3}/></button>
            </div>
            <div className="p-10 space-y-8">
              ...
              }} className="w-full py-5 bg-blue-600 text-white rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest shadow-xl border-b-4 border-blue-800">Partilhar Recibo</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border-2 border-slate-200 overflow-hidden transform-gpu">
        <div className="p-8 sm:p-10 border-b-2 border-slate-100 bg-slate-50/20">
          ...
          <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end">
            ...
            <button type="submit" className="bg-red-600 text-white rounded-2xl font-black text-xs py-5 shadow-xl border-b-4 border-red-800 active:scale-95 transition-all">Salvar Registro</button>
          </form>
        </div>
        ...
        <div className="lg:hidden p-6 space-y-6 bg-slate-50/50 smooth-scroll-container">
